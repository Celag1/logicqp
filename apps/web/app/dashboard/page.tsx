...
const SalesChart = lazy(() => import("@/components/ui/sales-chart").then(module => ({ default: module.SalesChart })));
const ProductsChart = lazy(() => import("@/components/ui/products-chart").then(module => ({ default: module.ProductsChart })));
const InventoryChart = lazy(() => import("@/components/ui/inventory-chart").then(module => ({ default: module.InventoryChart })));
import { 
  TrendingUp, 
  TrendingDown, 
  DollarSign, 
...
export default function DashboardPage() {
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      // Cargar productos con consulta optimizada
      ...
      const stockBajo = productos?.filter((p: any) => 
        (p.lotes?.[0]?.cantidad_disponible || 0) <= (p.stock_minimo || 0)
      ).length || 0;
      ...
      const realTopProducts: ProductData[] = productos?.slice(0, 5).map((producto: any) => ({
        ...
        ventas: Math.floor(Math.random() * 100) + 50, // Simular ventas
        crecimiento: Math.floor(Math.random() * 20) + 5, // Simular crecimiento
        categoria: producto.categorias?.nombre || 'Sin categoría',
        porcentaje: Math.floor(Math.random() * 15) + 5 // Simular porcentaje
      })) || [];
