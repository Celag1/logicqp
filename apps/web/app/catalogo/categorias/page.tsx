...
export default function CategoriasPage() {
  ...
  const loadCategories = async () => {
    try {
      ...
      const { data: categorias, error: categoriasError } = await Promise.race([
        queryPromise,
        timeoutPromise
      ]) as any;
      ...
      const realCategories: Category[] = categoriesWithStats.map((categoria: any, index: number) => {
        const productCount = categoria.productCount || 0;
        const averagePrice = categoria.averagePrice || 0;
      ...
      // Fallback: mostrar categorías básicas si falla la consulta
