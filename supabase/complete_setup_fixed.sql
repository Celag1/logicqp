-- =====================================================
-- LOGICQP - CONFIGURACIÓN COMPLETA AUTOMATIZADA (CORREGIDA)
-- =====================================================
-- EJECUTAR ESTE ARCHIVO DESPUÉS DE 000_complete_schema.sql
-- =====================================================

-- Eliminar triggers existentes si existen
DROP TRIGGER IF EXISTS tg_profiles_on_signup ON auth.users;
DROP TRIGGER IF EXISTS tg_auditoria_profiles ON profiles;
DROP TRIGGER IF EXISTS tg_auditoria_productos ON productos;
DROP TRIGGER IF EXISTS tg_auditoria_ordenes ON ordenes;

-- Eliminar funciones existentes si existen
DROP FUNCTION IF EXISTS tg_profiles_on_signup();
DROP FUNCTION IF EXISTS tg_auditoria_on_change();
DROP FUNCTION IF EXISTS fn_my_role();
DROP FUNCTION IF EXISTS fn_set_user_role(UUID, rol_type);
DROP FUNCTION IF EXISTS fn_checkout(INTEGER, TEXT, TEXT);
DROP FUNCTION IF EXISTS fn_registrar_compra(INTEGER, DATE, JSONB);
DROP FUNCTION IF EXISTS fn_low_stock_alerts();
DROP FUNCTION IF EXISTS fn_dashboard_metrics(DATE, DATE);
DROP FUNCTION IF EXISTS fn_log_event(TEXT, TEXT, TEXT, JSONB, JSONB);

-- Eliminar vistas existentes si existen
DROP VIEW IF EXISTS vw_kardex_producto;
...
-- Limpiar permisos existentes e insertar nuevos
DELETE FROM permisos WHERE rol IN ('super_admin', 'administrador', 'vendedor', 'inventario', 'contable', 'cliente');
...
-- =====================================================
-- INSERTAR DATOS DE PRUEBA (ORDEN CORRECTO)
-- =====================================================

-- PRIMERO: Limpiar datos existentes en orden correcto
DELETE FROM lotes;
DELETE FROM productos;
DELETE FROM proveedores;
DELETE FROM categorias;

-- SEGUNDO: Insertar categorías de productos farmacéuticos
...
-- CUARTO: Insertar productos Qualipharm (30 productos)
...
-- QUINTO: Insertar lotes para los productos
